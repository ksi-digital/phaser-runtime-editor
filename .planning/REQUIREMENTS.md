...
- [x] **COORD-03**: Gizmo drags must use a viewport snapshot captured at drag start (no mid-drag jitter)
- [x] **COORD-04**: Hit area transform logic must be centralized in CoordinateSystem (remove 3-file duplication)
- [x] **COORD-05**: Matrix inversion for Container children must be cached at drag start, not per-frame
